...
## 理解输入（Phase 0，搜索前必须完成）

> **铁律**：不理解需求就搜索 = 搜索方向跑偏。搜索关键词和对比维度都来自对 clarify 的理解。

### 读取并理解 clarify 文档

不是扫一眼文件，而是系统性理解：

1. **理解每条规则**：这个功能要做什么、不做什么
2. **理解正例和反例**：具体场景是什么，边界在哪
3. **理解排除项**：哪些明确不做
4. **理解约束**：技术栈限制、性能要求等
...
基于理解，明确：
- **要搜索什么**：从规则中提取核心技术问题（而非用户原始描述的泛化关键词）
- **对比什么维度**：从规则的约束推导对比标准（而非通用的性能/社区/星标数）
- **排除什么方案**：从排除项和约束直接排除不可行的方向

```
示例：
clarify 规则："通知只在用户打开页面时拉取，无需实时推送"
排除项："不做 WebSocket"
约束："技术栈 FastAPI + Vue3"

→ 搜索方向：轮询方案、懒加载通知列表、前端定时请求
→ 排除方向：WebSocket、SSE、Firebase Cloud Messaging
→ 对比维度：轮询间隔对用户体验的影响、服务端负载
```
...
> **铁律**：推荐方案前，必须用 clarify 的规则和示例验证可行性。

### 规则覆盖验证

用 clarify 中的每条规则和示例，验证推荐方案能否支撑：

```markdown
## 方案可行性验证

| clarify 规则 | 推荐方案是否支撑 | 说明 |
|-------------|----------------|------|
| 规则 1: [名称] | ✅ 支撑 | [具体说明] |
| 规则 2: [名称] | ✅ 支撑 | [具体说明] |
| 规则 3: [名称] | ⚠️ 部分支撑 | [需要额外处理的部分] |
...
如果有规则无法被推荐方案支撑，必须在推荐理由中说明，并给出替代建议。
