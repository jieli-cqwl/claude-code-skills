...
> **并行策略**：将 10 个评审维度合并为 5 个互补的评审组，每组包含相关维度。
> 每个 Agent 负责一个评审组，组内维度紧密相关，组间职责互斥。

**执行方式**：使用 Task 工具同时启动 5 个 subagent（subagent_type=general-purpose）

#### 5 Agent 评审组分配

| Agent | 评审组 | 包含维度 | 独占检查范围 |
|-------|--------|---------|-------------|
| Agent 1 | **需求覆盖组** | 完整性 + 遗漏分析 | 对照 AC 逐条检查覆盖情况，找出未覆盖的场景和边界 |
| Agent 2 | **方案一致性组** | 一致性 + 可行性 | 检查方案内部矛盾、与规范冲突、技术可实现性、依赖可用性 |
| Agent 3 | **风险安全组** | 风险识别 + 安全性 | 并发/竞态风险、注入攻击、权限漏洞、数据泄露、第三方依赖风险 |
| Agent 4 | **工程质量组** | 技术债务 + 可维护性 + 性能 | 设计复杂度、模块耦合、查询效率、资源消耗、扩展难度 |
| Agent 5 | **用户视角组** | 用户体验 | 操作流程合理性、响应速度、错误提示友好度 |

**为什么从 10 合并为 5 组**：
- 完整性与遗漏分析高度相关（都在检查"缺了什么"）
- 一致性与可行性都在检查"方案能否落地"
- 风险与安全性都在分析"可能出什么问题"
- 技术债务、可维护性、性能都属于"工程质量"范畴
- 合并后每个 Agent 职责更明确，减少重复分析
...
**v1.5 变更**：
- 将 10 Agent 优化为 5 个评审组，每组包含相关维度
- 减少 Agent 数量，提升评审效率
- 每个 Agent 有明确的独占检查范围，避免重复评审
- 新增 Prompt 模板，明确禁止范围
**v1.4 变更**：
- 实现 10 Agent 并行评审架构
