...
📊 门控结果：
- Layer 1 单元测试：✅ [数量] passed, 覆盖率 [百分比]
- Layer 2 集成测试：✅ [数量] API 端点正常
- Layer 3 E2E 验收：✅ [数量] 用户流程走通（或 🔀 跳过：[原因]）

⚠️ Layer 定义（禁止篡改）：
- Layer 1 = 单元测试（不是代码审查、不是 AC 验证）
- Layer 2 = 集成测试（API/服务边界）
- Layer 3 = E2E 验收（用户视角端到端）

🔒 质量保证：
- E2E 作为最终仲裁：功能确实可用
- 测试数据隔离：不影响生产环境
- 截图证据：docs/测试文档/screenshots/ 目录
