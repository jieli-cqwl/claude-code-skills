...
description: |
  代码重构。自动激活场景：
  1) 用户说"重构"、"代码重构"、"优化代码结构"时
  2) 用户说"代码太乱了"、"需要整理"时
  3) 用户说"过度设计了"时
  遵循简单、合适、演化原则，让代码保持最佳状态。自动识别语言并应用对应的重构规则。
---
...
- 问题定位精确："`UserService.create_user()` 56 行，违反单一职责（同时处理验证、创建、通知）"
- 方案具体："拆分为 `validate_user_data()` + `create_user()` + `notify_user_created()`"
- 影响分析："3 个调用方需更新：`api/users.py:42`, `tests/test_users.py:15`, `scripts/seed.py:8`"
- 回归测试："现有 12 个测试用例覆盖，拆分后保持全部通过"

### 坏的重构建议

- 问题描述模糊："这个类太大了，应该拆分"（多大？拆成什么？）
- 方案笼统："建议使用设计模式优化"（哪个模式？怎么用？）
- 无影响分析（改完后不知道会不会破坏其他功能）
- 无回归保障（重构后测试是否通过？没说）
