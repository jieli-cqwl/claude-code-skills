...
| 根因 | 证据 | 优先级 |
|------|------|--------|
| 需求澄清质量不足 | /prd 只做"对齐理解"不做"挑战方向"，输出的 AC 是人可理解粒度但不是机器可无歧义执行粒度 | 最大因素 |
...
- **现状**：/prd 做的是"对齐理解"，不做"挑战方向"；输出的 AC 是人能理解的粒度，但不是 Claude Code 能无歧义执行的粒度
- **目标**：澄清完成后的文档，具体到 Claude Code 可以不问任何问题直接走完全流程
- **关键改变**：
  - /prd 需要具备反向挑战能力（质疑需求方向是否正确）
...
| 条目 | 说明 |
|------|------|
| 完整流程保留 | 不砍环节（prd → design → plan → run-plan → check → qa → fix → ship），让每个环节做到位 |
...
- **最终结果**：可运行的代码 + 测试通过报告
- **中间留痕**：每个环节的输出文档（prd doc → design doc → plan doc → 各阶段 report → qa report）
...
| 解法方向 | 精简配置（31→10 Skills） | 提升 prd 质量 + 自动化流程编排 + 配置质量提升 |
