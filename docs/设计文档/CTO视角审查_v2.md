...
本次审查的是一套 **Claude Code 人机协作模式重建** 方案——用 Shell 脚本（pipeline.sh）编排多个 Claude CLI SubAgent，实现 clarify→design→plan→implement→check→qa→fix→ship 的自动化开发流程。

方案的设计文档采用三层结构：
- **L2（原文）**：完整设计文档，2900 行，是所有内容的 Source of Truth
- **L0（概览）**：从 L2 提取的全局视图（架构、流程、原则、风险），约 250 行
- **L1（模块规格）**：从 L2 按主题拆分的 6 个模块文件，每个 200-600 行

L0+L1 合在一起覆盖 L2 的全部 216 个可交付物（已验证 100%）。拆分目的是避免一次性加载 2900 行导致 token 浪费，实施时按需加载 L1。
...
9. `~/.claude/skills/docs/设计文档/设计_人机协作模式重建.md` — L0+L1 的拆分来源，2900 行
