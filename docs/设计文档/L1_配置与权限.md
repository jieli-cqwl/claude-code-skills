...
# 质量门控
[引用自 pipeline-architecture Skill]

# 输出前自检清单
[引用自 pipeline-architecture Skill]
```

> SubAgent 文件保留角色身份、行为准则、负向约束、输入输出规范。方法论、质量门控标准、Few-shot 示例通过 skills 字段（如 arch-methodology、tdd-methodology）从知识层引入。控制在 < 3k tokens。
...
**环境变量传入示例**：
```bash
HUMAN_CHECKPOINT=false TOTAL_BUDGET=200 UNATTENDED_MODE=true TEST_CMD_ALLOWLIST="pytest,npm test" CLI_CMD=claude-codex \
...
| # | 验证项 | 验收标准 |
|---|--------|---------|
| 1 | `claude -p --permission-mode plan` 实测 | Designer/Planner 在 plan 模式下可正常读写 Handoff |
| 2 | 费用校准 | 3 次不同规模需求实测费用已记录，STEP_BUDGET/TOTAL_BUDGET 基于 P90 x 1.5/2 校准 |
| 3 | SubAgent prompt 交付 | 6 个 SubAgent prompt 通过骨架 review（5 段落验收标准） |
| 4 | preflight_check() | 覆盖 jq/bc/gtimeout/setsid 依赖，缺失任一阻断启动 |
| 5 | 冒烟测试 | 1 次全流程（clarify→ship）通过 |
| 6 | shellcheck | pipeline.sh 零 warning |
