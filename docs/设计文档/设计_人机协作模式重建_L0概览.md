...
| `--permission-mode plan` 非交互兼容性 | ✅ 通过 | 不卡住，读操作正常（需 `--add-dir`），写操作自动拒绝，exit code 0。`--max-turns` 建议 ≥ 10 |
...
|   +-- pipeline-designer.md           # skills: [pipeline-architecture]
|   +-- pipeline-planner.md            # skills: [pipeline-architecture, pipeline-review-standard]
|   +-- pipeline-implementer.md        # skills: [pipeline-tdd-methodology, pipeline-code-quality]
|   +-- pipeline-checker.md            # skills: [pipeline-code-quality, pipeline-review-standard]
|   +-- pipeline-qa.md                 # skills: [pipeline-qa-methodology]
|   +-- pipeline-fixer.md              # skills: [pipeline-tdd-methodology, pipeline-code-quality]
