...
│  bash: 检查文件 + 写入 .pipeline-progress-{feature}.json │
│  $CLI_CMD -p [planner]   → handoff_plan              │
│  bash: 检查文件 + 更新进度                             │
│  $CLI_CMD -p [implementer] → handoff_run             │
│  $CLI_CMD -p [checker]   → handoff_check             │
│  while fix_count <= 10:                              │
│      $CLI_CMD -p [qa]    → handoff_qa                │
│      bash: 验证 RESULT 行存在且唯一 + 判断 PASS/FAIL  │
│      ├─ PASS → 通知 → 退出                           │
│      └─ FAIL → $CLI_CMD -p [fixer] → [checker]      │
│               → fix_count++ → 更新进度                │
└──────────────────────────────────────────────────────┘
    ↑ 写入                    ↑ 读取
...
| Handoff 文件残留 | 上次 Pipeline 未清理 | `pipeline.sh` 启动时检查目录是否已有 Handoff 文件，提示用户确认 |
...
TOTAL_STEPS=6                         # design, plan, implement, check, qa, (fix)
...
${prompt}" \
  ...
  if [ -n "$missing_sections" ]; then
    update_progress "$step_name" "$step_index" "failed"
    notify_error "Pipeline" "${step_name} 输出缺少必要章节:${missing_sections}"
    exit 1
  fi
...
run_step "implement" 3 "pipeline-implementer.md" \
...
run_step "check" 4 "pipeline-checker.md" \
...
请读取 ${PIPELINE_DIR}/handoff_clarify.md 和 ${PIPELINE_DIR}/handoff_check.md，执行验收测试，输出到 ${PIPELINE_DIR}/handoff_qa.md
结果判定行格式必须为: RESULT: PASS 或 RESULT: FAIL" \
  ...
  # --- RESULT 行解析（健壮性三重检查） ---
  result_lines=$(grep -c "^RESULT:" "${PIPELINE_DIR}/handoff_qa.md" || true)

  if [ "$result_lines" -eq 0 ]; then
    # QA 未输出 RESULT 行（格式偏差/输出截断）→ 立即失败，不进 fix 循环
    update_progress "qa" 5 "failed"
    notify_error "Pipeline" "QA 输出缺少 RESULT 判定行，请检查 handoff_qa.md"
    echo "ERROR: handoff_qa.md 中未找到 ^RESULT: 行，QA Subagent 可能未按约定格式输出"
    exit 1
  fi

  if [ "$result_lines" -gt 1 ]; then
    # 多个 RESULT 行（QA 输出混乱）→ 立即失败，不冒险取其中一个
    update_progress "qa" 5 "failed"
    notify_error "Pipeline" "QA 输出包含多个 RESULT 行，请检查 handoff_qa.md"
    echo "ERROR: handoff_qa.md 中发现 ${result_lines} 个 ^RESULT: 行，应恰好 1 个"
    exit 1
  fi

  # 恰好 1 个 RESULT 行，安全判断
  if grep -q "^RESULT: PASS" "${PIPELINE_DIR}/handoff_qa.md"; then
  ...
  run_step "fix-${fix_count}" 5 "pipeline-fixer.md" \
    "请读取 ${PIPELINE_DIR}/handoff_qa.md 和 ${PIPELINE_DIR}/handoff_check.md，修复问题，输出到 ${PIPELINE_DIR}/handoff_fix_${fix_count}.md" \
...
| 决策 | 理由 |
|------|------|
| QA 结果用 `grep -q "^RESULT: PASS"` 解析 | bash 确定性判断，不依赖 LLM 解读。三重检查：RESULT 行存在 → 恰好 1 个 → 判断 PASS/FAIL。QA Subagent prompt 中约定输出格式 |
...
| QA 输出缺少 RESULT 行 | `grep -c "^RESULT:"` 返回 0 | 更新进度为 failed，发送通知，脚本退出（不进 fix 循环） |
| QA 输出多个 RESULT 行 | `grep -c "^RESULT:"` 返回 >1 | 更新进度为 failed，发送通知，脚本退出（不冒险取其中一个） |
...
| Handoff 缺少必要章节 | `grep` 检查「输入分析/决策/产出」标题 | 更新进度为 failed，发送通知，脚本退出 |
...
| Checker | 代码审查者 | 独立代码审计员，与开发团队无利益关系 | 五维检查缺一不可；每个判断必须有客观证据（命令输出/文件路径:行号） | 不修改代码、不替开发者辩护、不给模糊的"PASS" |
...
| Implement → Check | commit 列表、测试运行结果摘要、已知遗留问题、BLOCKED 任务 |
| Check → QA | 五维检查结果摘要、FAIL 项清单（含文件路径:行号） |
...
**Skill 入口**: `/qa`
**工具**: Read, Bash, Glob, Grep（只读，不能修改代码）
**输入**: `handoff_clarify.md`（需求文档）+ `handoff_check.md`
**输出**: `handoff_qa.md`
...
读取 handoff_check.md：Checker 报告 2 个测试 FAIL，但 QA 不以此为准，独立验证。

## 决策及理由
以 handoff_clarify.md 为唯一验收标准，不参考 plan/design。
独立执行端到端 curl 验证，不依赖 Checker 的测试结果。
...
（无输入分析、以 Check 结果代替独立验收、无逐条覆盖、无端到端验证）
```
...
`CLI_CMD` 是唯一通过环境变量传入的配置，因为它决定了用哪个 backend 执行（claude 原生 / codex / gemini），属于运行时选择而非固定配置。
...
**前置依赖**：需要 `jq`（`brew install jq`）。如果 `jq` 不存在，Status Line 静默为空，不影响 Pipeline 运行。
