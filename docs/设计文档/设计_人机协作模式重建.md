...
│  │  bash: 检查 test_pass_flag 物理文件 → PASS / FAIL     │  │
│  │  ├─ CHECK_PASS → 继续                                 │  │
...
# --- 辅助函数：解析客观测试结果（不再使用正则过滤 LLM 文本，需要物理验证文件） ---
parse_result() {
  if [ -f "test_pass_flag" ]; then
    rm -f "test_pass_flag"
    echo "PASS"
  else
    echo "FAIL"
...
【关键指导】：五维检查全部通过后，必须在项目根目录创建物理记录：\`touch test_pass_flag\`。脚本依赖此文件判定 PASS/FAIL，不解析文档中的文字！无此文件一律视为 FAIL。" \
    "${PIPELINE_DIR}/handoff_check.md"

  # 解析 Check 结果（检查 test_pass_flag 物理文件）
  ...
  case "$(echo "$check_result" | tr '[:lower:]' '[:upper:]')" in
    ...
    *)
      ...
      update_progress "check" $step_index "failed"
      notify_error "Pipeline" "Check 结果解析异常（test_pass_flag 状态未知），请检查 handoff_check.md"
...
【关键指导】：在任何测试环节，验证完全通过后必须在当前命令行目录下创建物理记录：\`touch test_pass_flag\`，脚本不再抓取大模型文档里的 PASS 词语！无此文件将作打回处理。" \
  ...
  case "$(echo "$result" | tr '[:lower:]' '[:upper:]')" in
    ...
    *)
      # 防御性分支：parse_result() 正常只返回 PASS/FAIL，此分支理论上不可达
      update_progress "qa" 5 "failed"
      notify_error "Pipeline" "QA 结果解析异常（test_pass_flag 状态未知），请检查 handoff_qa.md"
...
| **(P0 新增) 物理自证凭证拦截** | 抛弃自然语言内的文本 `PASS` 检测形式。所有自动化产物测试必须能在文件树生成客观的标的（如 `test_pass_flag`），彻底阻断 LLM 的虚假完工幻觉。 |
...
| **(P2 新增) 费用总控** | 增加 `TOTAL_BUDGET` 参数（默认 $200，临时安全网）。当前 `claude -p` 不输出 session cost，暂用步数 × `STEP_BUDGET` 估算，超过暂停通知用户。待 `claude -p` 支持 cost 输出后升级为实际费用累加 |
...
| Check RESULT 格式 | 统一由 `touch test_pass_flag` 颁发通行证，杜绝一切自我标榜文字 |
| Implement-Check 循环上限 3 轮 | Check 阶段问题是代码层面的，修复收敛快；3 轮已足够。QA 阶段是功能层面，上限 10 轮 |
| QA 结果解析 | bash 确定性查询是否有测试通过构建戳 `test_pass_flag` |
...
| QA 输出无 `test_pass_flag` 文件 | `parse_result` 检查物理文件是否存在 | 视为 FAIL，进入 fix 循环 |
...
| **(新增) Check/QA 结果解析异常** | `test_pass_flag` 文件状态未知（如被外部进程误删或权限异常） | 通知用户，退出（防御性分支，正常流程不会触发） |
...
6. 五维全部 PASS 时，是否在项目根目录执行了 `touch test_pass_flag`？（脚本依赖此物理文件判定 PASS/FAIL，不解析文档文字）
7. Handoff 中是否包含一行 `RESULT: PASS` 或 `RESULT: FAIL` 摘要？（供人阅读，非脚本解析依据）
...
6. 全部验收通过时，是否在项目根目录执行了 `touch test_pass_flag`？（脚本依赖此物理文件判定 PASS/FAIL，不解析文档文字）
7. Handoff 中是否包含一行 `RESULT: PASS` 或 `RESULT: FAIL` 摘要？（供人阅读，非脚本解析依据）
...
| `TOTAL_BUDGET` | 200.00（环境变量） | 总费用安全网（USD），当前为步数 × STEP_BUDGET 估算，超过时暂停通知用户。待 claude -p 支持 cost 输出后升级为实际费用 |
...
| QA 结果解析异常 | QA/Fix 循环（防御性分支） | "QA 结果解析异常（test_pass_flag 状态未知），请检查 handoff_qa.md" | Basso |
...
| Check 结果解析异常 | Implement-Check 循环（防御性分支） | "Check 结果解析异常（test_pass_flag 状态未知），请检查 handoff_check.md" | Basso |
...
3. 验证每步：
   ...
   - QA 结果是否通过 `test_pass_flag` 物理文件正确判定 PASS/FAIL
...
6. Check 和 QA 均通过 `test_pass_flag` 物理文件判定 PASS/FAIL，验证 `parse_result()` 共用逻辑正确消费文件（存在 → PASS + 删除；不存在 → FAIL）
