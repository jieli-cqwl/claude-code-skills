...
**验证 3**：`claude -p --permission-mode plan` 在非交互模式下正常执行 → ✅ 通过（2026-02-22 验证）。plan 模式在 `-p` 下**不会卡住**：读操作正常（需 `--add-dir` 指定项目目录），写操作被自动拒绝（`ExitPlanMode` 在非交互下自动被拒绝），exit code 始终为 0。完全适合作为只读角色（Designer、Planner）的权限控制方案。注意 `--max-turns` 建议 ≥ 10（`3` 偏紧可能触发 `error_max_turns`）。
...
- ~~macOS 无 timeout~~：通过安装 `coreutils` 引用 `gtimeout --signal=KILL`，配合 `setsid` 进程组隔离，实施全进程树强杀。注意：`setsid` 在 macOS 上通过 `brew install util-linux` 安装后为 keg-only，不在默认 PATH 中。pipeline.sh 需使用完整路径 `/opt/homebrew/opt/util-linux/bin/setsid` 或在脚本头部添加 `export PATH="/opt/homebrew/opt/util-linux/bin:$PATH"`。
...
|----------|-------------|--------|-------|----------|--------|
| pipeline-designer | Read, Write, Glob, Grep, WebSearch | `[pipeline-architecture]` | `opus` | 30 | `project` |
| pipeline-planner | Read, Write, Glob, Grep | `[pipeline-architecture, pipeline-review-standard]` | `opus` | 30 | `project` |
| pipeline-implementer | Read, Write, Edit, Bash, Glob, Grep | `[pipeline-tdd-methodology, pipeline-code-quality]` | `opus` | 50 | `project` |
| pipeline-checker | Read, Bash, Glob, Grep | `[pipeline-code-quality, pipeline-review-standard]` | `sonnet` | 20 | - |
| pipeline-qa | Read, Bash, Glob, Grep | `[pipeline-qa-methodology]` | `sonnet` | 30 | - |
| pipeline-fixer | Read, Write, Edit, Bash, Glob, Grep | `[pipeline-tdd-methodology, pipeline-code-quality]` | `opus` | 40 | - |
...
> 注意：角色身份、行为准则、负向约束、输入输出规范保留在 SubAgent 文件中。方法论（如何扫描代码、如何做多方案对比）、质量门控详细标准、Few-shot 示例等方法论性质的内容通过 `skills` 字段（如 arch-methodology、tdd-methodology）从知识层引入。这将 SubAgent 文件控制在 < 3k tokens。
