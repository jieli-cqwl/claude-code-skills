...
│  $CLI_CMD -p [designer]  → handoff_design            │
│  bash: 检查文件 + 写入 .pipeline-progress-{feature}.json │
│  $CLI_CMD -p [plan_summary] → Key_Decisions.md       │ 
│  bash: 暂停等待人工 Review (支持交互式调整)             │
...
HUMAN_CHECKPOINT=${HUMAN_CHECKPOINT:-true}  # 默认开启 Design/Plan 人工确认（方向性决策必须人审核）
...
1. 验收标准来源是否为 handoff_clarify.md？（不是 plan/design）
...
| `HUMAN_CHECKPOINT` | `true`（环境变量） | Design/Plan 人工确认开关，默认开启 |
