...
| 概念 | 粒度 | 用途 | 格式 |
|------|------|------|------|
| **Task** | 可配置（默认 15-30 分钟） | /run-plan 执行单元 | `- [ ] T1: 描述` |
...
| 项目类型 | Task 粒度 | 说明 |
|---------|----------|------|
| 快速迭代 | 10-20 分钟 | 更细粒度，快速验证 |
| 标准开发 | 15-30 分钟 | 默认推荐 |
| 复杂系统 | 30-60 分钟 | 大型功能，减少调度开销 |
...
> 在选择拆分模式前，必须完成以下 4 维度分析：

| 维度 | 分析内容 | 输出 |
|------|---------|------|
| **功能独立性** | 功能数量、文件独立性 | 功能列表 + 文件范围 |
| **数据依赖** | 共享数据表、读写顺序 | 数据依赖图 |
| **时间前置** | 哪些必须先完成 | 关键路径 |
| **通信成本** | 功能间 API 调用关系 | 接口依赖图 |
...
### 1. 功能独立性分析
- 功能 A: 用户登录 → `login.py`, `LoginForm.tsx`
- 功能 B: 用户注册 → `register.py`, `RegisterForm.tsx`
- 功能 C: 密码重置 → `reset.py`, `ResetForm.tsx`
- **结论**：3 个独立功能，文件无重叠 ✅

### 2. 数据依赖分析
- 功能 A/B/C 都读写 `users` 表
- 但操作不冲突：A 读取、B 插入、C 更新
- **结论**：数据依赖弱，可并行 ✅

### 3. 时间前置分析
- `users` 表 Schema 必须先存在
- JWT 中间件必须先配置
- **结论**：T0 基础设施是关键前置

### 4. 通信成本分析
- 功能 A/B/C 之间无直接 API 调用
- 都依赖 T0 的基础设施
- **结论**：通信成本低，适合并行 ✅
...
> **AC 单一来源**：`docs/需求澄清/clarify_[功能名].md` 中的 AC 表格
> **用途**：/qa 执行时基于此场景验收

**引用方式**（强制格式，P1 增强）：

> ⚠️ **AC 引用格式强制化**：必须使用 `[AC-N → 来源文档]` 格式，确保溯源可追溯

```markdown
**AC 来源文档**：`docs/需求澄清/clarify_用户认证.md`
**AC 指纹**：AC-HASH-1A2B3C4D（由 /clarify 生成，用于防篡改检测）

**引用的 AC 列表**（使用强制格式）：
- [AC-1 → clarify_用户认证.md] 正确密码登录 → 跳转首页
- [AC-2 → clarify_用户认证.md] 注册成功 → 自动登录
- [AC-3 → clarify_用户认证.md] 错误密码 → 显示密码错误
- [AC-4 → clarify_用户认证.md] 邮箱未注册 → 显示用户不存在
- [AC-5 → clarify_用户认证.md] 密码最小长度 → 注册成功
- [AC-6 → clarify_用户认证.md] 密码过短 → 显示错误

**完整 AC 表格见**：[clarify_用户认证.md](docs/需求澄清/clarify_用户认证.md#4-验收标准)
```

**格式校验**：
- ✅ `[AC-1 → clarify_xxx.md]` - 正确格式
- ❌ `AC-1: xxx` - 错误，缺少来源文档
- ❌ `验收标准 1: xxx` - 错误，未使用 AC 编号
...
- [ ] AC 来源文档存在：`docs/需求澄清/clarify_[功能名].md`
...
### 拆分模式检查
- [ ] 完成了 **4 维度拆分决策分析**（功能独立性、数据依赖、时间前置、通信成本）
- [ ] 分析了功能数量和文件独立性
- [ ] 选择了合适的拆分模式（多人协作/依赖链）
- [ ] 写明了选择原因
- [ ] 包含 **ASCII 依赖关系图**

### 验收测试场景检查（AC 引用，P1 强化）
- [ ] 有"验收测试场景"章节
- [ ] **引用** /clarify 的 AC 表格（禁止重新定义）
- [ ] **使用强制引用格式**：`[AC-N → 来源文档]`
- [ ] **包含 AC 指纹**（来自 /clarify 生成）
- [ ] AC 来源文档路径正确
...
📋 /clear 后可直接执行（复制粘贴）：
   /test-gen from-clarify docs/需求澄清/clarify_[功能名].md
