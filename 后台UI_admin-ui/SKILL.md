...
- 搜索栏：支持多字段组合搜索，回车触发
- 表格：分页（默认 20 条/页）、列排序、行选择（支持批量操作）
- 空状态：图标 + 文案 + 创建按钮引导
- 错误状态：行内 toast 提示，不阻断页面
- 操作列：查看/编辑/删除，删除有二次确认

### 坏的 CRUD 表格页

- 无分页，一次加载全部数据（数据量大时页面卡死）
- 无空状态处理（表格直接显示空白，用户不知道是没数据还是加载失败）
- 删除无确认弹窗（误触直接删除，无法恢复）
- 搜索后不重置分页（显示"无数据"但实际是在第 N 页搜索）
