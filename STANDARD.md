...
/design（架构设计）
    ↓ 设计完成后
/plan（写计划 v3.0）← 支持多人协作拆分
    ↓
/run-plan（执行计划 v3.0）← 多人协作执行，效率提升 2-5 倍
    ├── Tech Lead 前置工作（基础设施）
    ├── Alice/Bob/Charlie 并行开发（严格 TDD）
    └── Tech Lead 后置工作（集成 + 服务启动验证）
    ↓ 开发完成后
/check（开发检查 v3.0）← 6 Agent 并行，含服务启动验证
    ↓ 检查通过后
/qa（测试验收）
    ↓ 测试通过后
/ship（代码交付）
...
**核心改进**：
- `/plan` v3.1：支持多人协作模式 + **验收测试场景**（Given-When-Then）
- `/run-plan` v3.0：多人协作执行，效率提升 2-5 倍
- `/check` v3.0：新增服务启动验证（Agent6）
- `/qa` v3.3：**基于 /plan 验收场景执行验收**

**可选分支**：
- `/product`：涉及用户体验时，在 `/clarify` 之后使用
- `/refactor`：代码重构时，替代 `/explore` → `/design` → `/plan` 流程
- **`/auto-dev`**：自动循环开发，融合多人协作 + 严格 TDD
...
**三大原则**：
1. **简单**：不过度设计，只有一个实现的接口就删除
2. **合适**：该复杂时复杂，有多个实现需要切换就保留接口
3. **演化**：根据实际需求演化，需要时再加，不预设
