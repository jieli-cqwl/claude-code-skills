...
```bash
# 门控1: 检查 AC 来源文档
PRD_DOC=$(ls docs/pipeline/*/prd.md 2>/dev/null | head -1)
if [ -z "$PRD_DOC" ]; then
...
if [ -z "$TEST_FILE" ]; then
  echo "❌ 门控3失败: 测试文件不存在"
  echo "   修复: 执行 /test-gen from-prd $PRD_DOC"
  exit 1
else
...
门控1失败（AC 文档不存在）
    ↓
执行 /prd 生成需求文档
    ↓ 输出: docs/pipeline/{feature}/prd.md
...
门控3失败（测试文件不存在）
    ↓
执行 /test-gen from-prd docs/pipeline/{feature}/prd.md
