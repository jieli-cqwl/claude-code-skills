...
| 文档 | 路径 | 用途 |
|------|------|------|
| **AC 文档** | `docs/需求文档/clarify_[功能名].md` | AC 单一来源，后续所有环节依赖 |
...
需求澄清完成后，输出增强版文档并保存到指定路径。

**📁 文档保存路径**：`docs/需求文档/clarify_[功能名].md`

> 示例：`docs/需求文档/clarify_用户认证.md`
...
**澄清日期**：YYYY-MM-DD
**状态**：已确认
**文档路径**：docs/需求文档/clarify_[功能名].md
...
> **格式要求**：每个 AC 必须有唯一 ID，便于全流程追溯
> **覆盖要求**：正常流程 + 异常流程 + 边界情况
> **测试方法**：指导 /test-gen 生成对应类型的测试
...
| AC-ID | Given | When | Then | 优先级 | 测试方法 |
|-------|-------|------|------|--------|---------|
| AC-5 | [边界条件，如最大/最小值] | [边界操作] | [边界处理结果] | P2 | 单元测试 |
| AC-6 | ... | ... | ... | P2 | 属性测试 |

**测试方法说明**：
| 测试方法 | 适用场景 | 说明 |
|---------|---------|------|
| 单元测试 | 纯函数、边界值、异常处理 | 最快，覆盖细节 |
| 集成测试 | API 调用、数据库操作 | 验证组件协作 |
| E2E | 完整用户流程 | 最慢，验证端到端 |
| API 测试 | 接口参数、错误响应 | 验证 API 契约 |
| 属性测试 | 数学性质、不变量 | 自动生成边界用例 |
...
> **`/clarify` 的职责边界：只做需求澄清，不能跳过后续环节**

| 禁止行为 | 说明 |
|---------|------|
| ❌ 跳过 `/explore` 直接进入 `/design` | 必须按顺序：clarify → explore → design |
| ❌ 跳过整个流程直接修改代码 | 必须走完 explore → design → plan → run-plan |
| ❌ 在澄清阶段写任何实现代码 | 只输出 AC 文档，不输出代码 |

**正确的完成动作**：
1. 输出 AC 文档到 `docs/需求文档/clarify_[功能名].md`
2. 展示完成提示
3. 进入下一环节 `/explore`（正常流转）或等待用户指令

**跳过环节的处理**：
- 如果用户要求跳过某个环节（如"跳过 explore 直接 design"）
- 必须明确提醒用户跳过的风险，并获得确认
- 未经确认不能跳过
...
> 以上情况应直接修复，不启动 `/clarify`。判断标准：**是否需要设计决策**。
...
📁 文档已保存至：docs/需求文档/clarify_[功能名].md
...
5. 执行 /plan（写计划）- 制定详细实施步骤
6. 执行 /test-gen from-clarify - 从 AC 生成测试（开发前）
